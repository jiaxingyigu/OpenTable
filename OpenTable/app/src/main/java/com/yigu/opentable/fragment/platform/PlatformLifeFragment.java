 ...
 */
public class PlatformLifeFragment extends BaseFrag {
    ...
    public void load(){
        showLoading();
        CommonApi.knowledge(getActivity(),userSP.getUserBean().getUSER_ID(), "1", pageIndex + "", pageSize + "", new RequestPageCallback<List<MapiPlatformResult>>() {
            @Override
        ...
        mAdapter.setRecyOnItemClickListener(new RecyOnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                ...
                if(!TextUtils.isEmpty(url))
                    ControllerUtil.go2WebView(url,"平台信息详情","养生常识",mList.get(position).getTitle(), BasicApi.BASIC_IMAGE + mList.get(position).getPATH(),true);
            }
        });
