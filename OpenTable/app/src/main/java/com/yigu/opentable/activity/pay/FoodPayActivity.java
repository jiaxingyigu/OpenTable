...
public class FoodPayActivity extends BaseActivity {
    ...
    private void initView() {
        ...
        payWayLayout.setHiddenSend();
        payWayLayout.load(companyId);

        /*if(TextUtils.isEmpty(seat)){
            if(hasBZ) {
                payWayLayout.showBZ();
                payWayLayout.setBZTip("取货时间");
            }
...
//                if(TextUtils.isEmpty(seat)&&TextUtils.isEmpty(payWayLayout.getBz())){
//                    MainToast.showShortToast("请输入取货时间");
//                }else{
                    showLoading();
                    OrderApi.preorder(FoodPayActivity.this,companyId, userSP.getUserBean().getUSER_ID(), SHOP, allPrice+"", sales,payWayLayout.getBz(),seat, "",
                        ...
                        public void success(Object success) {
                            ...
                            try {
                                ...
                                if(TextUtils.isEmpty(seat))
                                    startActivity(new Intent(FoodPayActivity.this,OrderCompleteActivity.class));
                                else
                                    startActivity(new Intent(FoodPayActivity.this,OrderCompleteActivity.class).putExtra("back","food"));
                            } catch (DbException e) {
                                e.printStackTrace();
                            }
                        }
...
//                }else{
                    showLoading();
                    OrderApi.balancepay(FoodPayActivity.this,companyId, userSP.getUserBean().getUSER_ID(), SHOP, allPrice+"", sales,payWayLayout.getBz(),seat,"",
                            payWayLayout.getCtype1(),payWayLayout.getCtype2(),payWayLayout.getCtype3(),payWayLayout.getSendPay(), new RequestCallback() {
                        @Override
                        public void success(Object success) {
                            hideLoading();
...
//                }else{
                    ...
                    OrderApi.weixinPay(FoodPayActivity.this, companyId,userSP.getUserBean().getUSER_ID(), SHOP, allPrice + "", price +"",sales, payWayLayout.getBz(), seat,"",
...
//                }else{
                    showLoading();
                    OrderApi.zhifubaoPay(FoodPayActivity.this,companyId, userSP.getUserBean().getUSER_ID(), SHOP,  allPrice +"", sales, payWayLayout.getBz(),seat,"",
                            payWayLayout.getCtype1(),payWayLayout.getCtype2(),payWayLayout.getCtype3(),payWayLayout.getSendPay(),new RequestCallback<JSONObject>() {//
                        @Override
                        public void success(JSONObject success) {//
                            hideLoading();
    ...
    private void callPay(final String orderInfo){
       ...
       /* final String payInfo = orderInfo + "&sign=\"" + sign ;*/
        ...
        Runnable payRunnable = new Runnable() {
            ...
            public void run() {
                // 构造PayTask 对象
                PayTask alipay = new PayTask(FoodPayActivity.this);
                // 调用支付接口，获取支付结果
                ...
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                       /* showLoading();
                        ...
                        try {
                            Toast.makeText(FoodPayActivity.this, "支付成功", Toast.LENGTH_SHORT).show();
                            db.delete(MapiOrderResult.class);
                            if(TextUtils.isEmpty(seat))
                                startActivity(new Intent(FoodPayActivity.this,OrderCompleteActivity.class));
                            else
...
//					Toast.makeText(PayActivity.this, "支付成功", Toast.LENGTH_SHORT).show();
                        ...
                        if (TextUtils.equals(resultStatus, "8000")) {
                            Toast.makeText(FoodPayActivity.this, "支付结果确认中", Toast.LENGTH_SHORT).show();
                            ...
                            Toast.makeText(FoodPayActivity.this, "支付失败", Toast.LENGTH_SHORT).show();
