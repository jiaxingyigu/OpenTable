...
public class FoodPurcaseActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_food_purcase);
...
//                if(TextUtils.isEmpty(seat)&&TextUtils.isEmpty(payWayLayout.getSendPay())){
//                    MainToast.showShortToast("请输入取货时间");
//                }else{
                    showLoading();
                    OrderApi.preorder(FoodPurcaseActivity.this,companyId, userSP.getUserBean().getUSER_ID(), SHOP, allPrice+"", sales, payWayLayout.getBz(),seat,"",
                        ...
                        public void success(Object success) {
                            ...
                            try {
                                ...
                                if(TextUtils.isEmpty(seat))
                                    ...
                                    startActivity(new Intent(FoodPurcaseActivity.this,OrderCompleteActivity.class).putExtra("back","food"));
                            } catch (DbException e) {
                                e.printStackTrace();
                            }
                        }
...
//                }else{
                    showLoading();
                    OrderApi.balancepay(FoodPurcaseActivity.this,companyId, userSP.getUserBean().getUSER_ID(), SHOP, allPrice+"", sales,payWayLayout.getBz(),seat,"",
                            payWayLayout.getCtype1(),payWayLayout.getCtype2(),payWayLayout.getCtype3(),payWayLayout.getSendPay(),new RequestCallback() {
                        @Override
                        public void success(Object success) {
                            hideLoading();
...
//                }else{
                    ...
                    OrderApi.weixinPay(FoodPurcaseActivity.this, companyId,userSP.getUserBean().getUSER_ID(), SHOP, allPrice + "", price +"",sales, payWayLayout.getBz(),seat,"",
...
//                }else{
                    showLoading();
                    OrderApi.zhifubaoPay(FoodPurcaseActivity.this,companyId, userSP.getUserBean().getUSER_ID(), SHOP,  allPrice +"", sales, payWayLayout.getBz(),seat,"",
                            payWayLayout.getCtype1(),payWayLayout.getCtype2(),payWayLayout.getCtype3(),payWayLayout.getSendPay(),new RequestCallback<JSONObject>() {//
                        @Override
                        public void success(JSONObject success) {//
                            hideLoading();
    ...
    private void callPay(final String orderInfo){
       ...
       /* final String payInfo = orderInfo + "&sign=\"" + sign ;*/
        ...
        Runnable payRunnable = new Runnable() {
            ...
            public void run() {
                // 构造PayTask 对象
                PayTask alipay = new PayTask(FoodPurcaseActivity.this);
                // 调用支付接口，获取支付结果
...
//					Toast.makeText(PayActivity.this, "支付成功", Toast.LENGTH_SHORT).show();
                        ...
                        if (TextUtils.equals(resultStatus, "8000")) {
                            Toast.makeText(FoodPurcaseActivity.this, "支付结果确认中", Toast.LENGTH_SHORT).show();
                            ...
                            Toast.makeText(FoodPurcaseActivity.this, "支付失败", Toast.LENGTH_SHORT).show();
